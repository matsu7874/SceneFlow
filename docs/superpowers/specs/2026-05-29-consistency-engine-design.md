...
**対人行動の共通規則**: `type` に関わらず `interactedPersonId` が指定された Act は、対象との共在 `at(interactedPersonId, locationId)` を追加 require とする（その場にいない相手は殺害・発砲・拘束・受け渡し・会話できない）。「殺害した／発砲した／拘束した」等は description 自由記述＋`interactedPersonId`（＋武器があれば `propId`）で表現し、共在チェックの対象になる。`GIVE`/`SPEAK` の対象 require はこの共通規則の特例。

**v1 で追わないこと**: 対人行動の結果としての対象の状態変化（死亡・拘束による以降の行動制約など）は検査しない。WorldState は位置・所持・知識の 3 次元のみ。人物の condition 次元（生存/拘束等）は将来拡張に委ねる（決定: v1 は共在チェックのみ）。
...
**QuickLog は「破綻を気にせずどんどん要素を入れる」ための高速入力ページであり、修正の場ではない。** 整合性の警告は入力の邪魔をしない受け身の表示に留め、破綻を腰を据えて直す導線（一覧・グラフ・誘導）は置かない。修正は検証ページと因果ビューが担う。
...
- `ActTimelineRow` は当該 Act の `Breakage[]` を受け取り、**行末に小さく控えめなアイコン**（カテゴリ別: 位置/分身/物/情報）を 1 つ表示する。`title`（ツールチップ）に件数と `message`。複数カテゴリは小さなアイコンを並べるか件数バッジ 1 つに集約し、テキストや色で目立たせない（入力のテンポを乱さない）。クリックでの修正 UI は出さない（詳細パネルは従来どおり行クリックで開く肉付け用）。
...
- 単体（`tests/modules/consistency/`）:
  ...
  - `checker`（4 カテゴリ＋来歴）:
    ...
    - 対人共在: 種別「その他」でも `interactedPersonId` があれば対象の共在を require とし、非共在を破綻（例: その場にいない相手を「拘束した」）
...
- コンポーネント:
  ...
  - `ActTimelineRow`: 与えた Breakage に応じ行末に控えめなアイコン（件数/ホバー説明）、修正 UI は出さない
...
2. QuickLog タイムラインで各破綻が**行末の小さく控えめなアイコン**（ホバーで説明）として出る。QuickLog には修正導線を置かず、高速入力を妨げない。
