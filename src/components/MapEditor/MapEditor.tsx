...
export const MapEditor: React.FC<MapEditorProps> = ({
  ...
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; type: 'node' | 'connection' | 'canvas'; target?: any } | null>(null);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (e.button === 2) {
      ...
      if (node) {
        ...
        setContextMenu({ x: e.clientX, y: e.clientY, type: 'connection', target: connection });
      } else {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.controls}>
        ...
        <div className={styles.controlGroup}>
          ...
          <button 
            className={styles.button}
            onClick={() => {
              const data = prompt('Paste map data:');
              if (data) {
      ...
      <div className={styles.stats}>
        <div>Nodes: {editor.state.mapData.locations.length}</div>
        <div>Connections: {editor.state.mapData.connections.length}</div>
        <div>Selected: {editor.state.selection.selectedNodes.size}</div>
      </div>
      ...
      {editor.state.pathfinding.active && editor.state.pathfinding.path && (
        <div className={styles.pathInfo}>
          <h4>Path Found</h4>
          <div>Steps: {editor.state.pathfinding.path.length}</div>
          <div>
            Cost: {(() => {
        ...
        >
          {contextMenu.type === 'canvas' && (
            <>
              <div 
                ...
                onClick={() => {
                  const rect = editor.canvasRef.current?.getBoundingClientRect();
                  if (rect) {
          ...
          {contextMenu.type === 'node' && (
            <>
              ...
              >
                {!editor.state.pathfinding.start ? 'Set as Path Start' : 
                 editor.state.pathfinding.start === contextMenu.target.id ? 'Path Start' : 'Set as Path End'}
              </div>
      ...
      <div className={styles.helpText}>
        Double-click to add node • Ctrl+drag to connect • Shift+drag to multi-select • Right-click for options
      </div>
    </div>
  );
};