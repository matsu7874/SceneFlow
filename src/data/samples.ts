...
import academyMystery from './sample-academy-mystery.json'
import mansionMystery from './sample-mansion-mystery.json'

/**
 * 物語サンプルの共通定義。
 * オンボーディング・空状態・JSON入力など複数箇所から同じサンプルを参照できるようにする。
 * （以前は JsonDataInput 内にハードコードされ、他から呼べなかった）
 */
...
// 桃太郎サンプル（旧 JsonDataInput.defaultJsonData を移設）。
// props の category などは型の enum と一致しないため、StoryData へはキャストして扱う。
const momotaro = {
  persons: [
    {
      id: 1,
...
export const SAMPLES: StorySample[] = [
  {
    id: 'momotaro',
    label: '桃太郎',
    description: '基本機能を試せる素朴な冒険譚。複数人物の合流と移動が学べます。',
    data: momotaro as unknown as StoryData,
  },
  {
    id: 'academy',
    label: '学院ミステリ',
    description: '証言・アリバイが絡む学院もの。検証や容疑者・機会の分析向き。',
    data: academyMystery as unknown as StoryData,
  },
  {
    id: 'mansion',
    label: '嵐の洋館ミステリ',
    description: '閉鎖環境のマーダーミステリ。因果・矛盾の検出を試せます。',
    data: mansionMystery as unknown as StoryData,
  },
]
