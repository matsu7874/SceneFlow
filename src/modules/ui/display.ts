...
export function updateUI(currentMinutes: number): void {
  ...
  currentMinutes = Math.max(
    simState.minTimeMinutes,
    Math.min(currentMinutes, simState.maxTimeMinutes),
  )
  ...
  const indexedData = simState.indexedData
  if (indexedData.persons.length === 0) {
    domElements.locationOutput.textContent = '登場人物がいません'
  } else {
    indexedData.persons.forEach(person => {
      const personColor = person.color || '#000000'
      const state = worldState[person.id]
      const locationId = state ? state.locationId : null
      const location = locationId ? indexedData.locationMap.get(locationId) : null
  ...
  const filteredEntries = simState.eventLogEntries.filter(
    entry => entry.timeMinutes <= currentMinutes,
  )
